...
import { AlertTriangle, CheckCircle2, FileUp, Rocket } from "lucide-react";
...
export default function Upload() {
  ...
  const [appId, setAppId] = useState<number | null>(
    appIdParam ? Number(appIdParam) : null,
  );
  ...
  useEffect(() => {
    ...
    getOwnedAppDetail(appIdParam)
      .then(setApp)
      .catch((err) =>
        setError(extractErrorMessage(err, "Could not load the app")),
      )
  ...
  const handleUploadVersion = async (e: FormEvent) => {
    ...
    try {
      const result = await uploadVersion(appId, file, versionTag, setProgress);
      setCompatibilityWarning(result.compatibilityWarning);
      setUploaded(true);
    } catch (err) {
      ...
      {uploaded && (
        <div className="flex flex-col items-center gap-4 bg-surface-container-lowest border border-outline-variant/60 rounded-xl p-8 text-center">
          ...
          {compatibilityWarning && (
            <div className="w-full flex items-start gap-2 bg-error-container/60 border border-error/30 text-error rounded-lg px-4 py-3 text-sm font-sans text-left">
